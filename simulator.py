...
class BlackHoleRenderer:
    """
    Base renderer used by all four black hole animations.
    """
    ...
    def __init__(
        ...
        self.theta = np.linspace(0, 2 * np.pi, 600)
    ...
    def add_starfield(self, fig, number=350):
        ...
        sizes = np.random.uniform(1.0, 3.8, number)
        opacity = np.random.uniform(0.35, 1.0, number)
    ...
    def add_nebula(self, fig):
        ...
        for colour in nebula_colours:
            ...
            x = np.random.normal(0, 2.4, 1600)
            y = np.random.normal(0, 2.4, 1600)
    ...
    def add_galaxy(self, fig):
        ...
        theta = np.linspace(0, 8 * np.pi, 2500)
        radius = 0.16 * theta
    ...
    def add_legend(
        ...
        if disk:
            legend += "🔵 Accretion Disk<br>"

        if ergosphere:
            legend += "🟠 Ergosphere<br>"

        if electric:
            legend += "⚡ Electric Field<br>"
    ...
    def orbit_frame(self, angle, radius):
